...
class pop3_class
{
	...
	/**
	 * Function getArrayHeaders
	 *
	 * @return Array - Las cabeceras del mensaje.
	 */
	function getArrayHeaders( $mensaje ) {
		$this->RetrieveMessage( $mensaje, $headers, $body, 100 );
		
		for( $i=0; $i<count($headers); $i++ ) {
			$_array = preg_split("[:]", $headers[$i], 2);
			$return_headers[trim($_array[0])] = trim($_array[1]);
		}
	...
	/**
	 * Function getArrayBody
	 *
	 * @return Array - El cuerpo del mensaje.
	 */
	function getArrayBody( $mensaje ) {
		$this->RetrieveMessage( $mensaje, $headers, $body, 100 );
				
		$campos = array("FAT", "EMPLZ", "F", "AR", "NGN", "CRT", "OBS", "ACT", "SACT", "FAR", "FPR",
						"FII", "FR", "AR", "FR", "AR", "NAM", "FIT", "NCTZ", "UFT", "NUMV");

		$body = $this->inArrayReplace("<br>", "", array_map("trim", $body));	
		list( $comienzo, $fin ) = array_keys( $body, "#########################################");
		
		for($i=$comienzo+1, $x=0; $i<$fin; $i++, $x++ ) {
			$aux[$x] = $body[$i];
		}
