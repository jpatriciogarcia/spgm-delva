...
"NCTZ"	=> "^[[0-9]{1,10}|-[0-9]{1,10}]$",						// NUM(10)
"UFT"	=> "^[[0-9]{1,7}|[0-9]{1,7}[|.[0-9]{1,2}|,[0-9]{1,2}]]$",			// NUM(7,2) => (1234567,89)
"OBS"	=> "^.{1,255}$"						 					// CHAR(255)
),
"F4" => array(
"FAT"	=> "^[0-9]{1,10}$", 									// NUM(10)
...
function ValidaFormato( $arr_valores, $arr_formato) {
	if( array_diff_assoc( array_keys($arr_formato), array_keys($arr_valores) ) ) {
		if($GLOBALS['debug']) echo "<hr />(".$_SERVER['PHP_SELF']." línea 77):<br /><hr />";
		return 8;
	}
	foreach( $arr_valores as $k => $v ) {
		if( array_key_exists($k, $arr_formato) ) {
			if( ! ereg($arr_formato[$k], trim($v)) ) {
				if($GLOBALS['debug']) echo "<hr />(".$_SERVER['PHP_SELF']." línea 83):<br /> [$k] => $v<hr />";
				return 8;
			}
		} else {
			if($GLOBALS['debug']) echo "<hr />(".$_SERVER['PHP_SELF']." línea 87):<br /> [$k] => $v<hr />";
			return 8;
		}
	}
	return false;
	...
	if( ($error=$pop3->RetrieveMessage($i,$headers,$body,100))=="" ) {
		...
		if( $ProveedorCII == _SIGLA_PROVEEDOR_ ) {
			...
			elseif( $TipoTrabajo == "TE" ) {
				...
				// Acepta, Rechaza
				// Formato Mail: FF3
				elseif( $CodigoAccion == "TAC" && !($CodigoError = ValidaFormato($body, $formato["FF3"])) ) {
					list($fecha, $hora) = explode(" ", $body["FR"], 2);
				...
				// Rechaza
				// Formato Mail: FF5
				elseif( $CodigoAccion == "TRC" && !($CodigoError = ValidaFormato($body, $formato["FF5"])) ) {
					list($fecha, $hora) = explode(" ", $body["FR"], 2);
		...
		elseif( $ProveedorCII!='TEST' && $ProveedorCII!='ISO' ){
			$CodigoError = 6;
			// En este paso grabamos el mensaje en un archivo para tenerlo como backup
			$archivo = _PATH_RELATIVO_ . "/integracion/mails/" . date("YmdHis") . "_" . $headers["Subject"];
			GrabarArchivo($archivo);
		...
		switch( $CodigoError && $TipoTrabajo!="ERR") {
			...
			$msgError .= "Se ha enviado una notificaci�n a " . _INTEGRADOR_ . " con los errores.";
